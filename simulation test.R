...
for (n in seq_along(Nlist)) {
  for (t in seq_along(Tlist)) {
    ...
    res <- foreach(k = 1:Rep, .combine = 'rbind', .errorhandling = 'remove', .packages = c("gtools", "sandwich", "plm", 'pcluster')) %dorng% {
      ...
      # Baseline estimate
      est <- estimator_dc(formula, data, index = c('id', 'time'), cluster_type = 'kcenter')
      ols <- est[["res"]]
      G <- est[["G"]]
      C <- est[["C"]]
      ...
      est_CF <- estimator_dc(formula, data, CF = TRUE, index = c('id', 'time'), cluster_type = 'kcenter')
      ols_CF <- est_CF[["res"]]
